...
class Solution
  ...
  def load_collection(file_path)
    self.class.collection.insert_many(self.class.load_hash(file_path))
  end
  ...
  def find_by_name(fname, lname)
    self.class.collection.find(first_name: fname, last_name: lname).projection(first_name: 1, last_name: 1, number: 1, _id: 0)
  end
  ...
  def find_group_results(group, offset, limit)
    self.class.collection.find(group: group).limit(limit).skip(offset).sort(secs: 1).projection(group: 0, _id: 0)
  end
  ...
  def find_between(min, max)
    self.class.collection.find(secs: {:$lt => max, :$gt => min})
  end

  def find_by_letter(letter, offset, limit)
    self.class.collection.find(last_name: { :$regex => "^#{letter.upcase}"}).skip(offset).limit(limit).sort(last_name: 1)
  end
  ...
  def add_time(number, secs)
    self.class.collection.find(number: number).update_many(:$inc => {secs: secs})
  end
