...
class Assignment
  ...
  def create_todolist(params)
      ...
      # return an instance of the class with primary key (`id`), and dates (`created_at` and `updated_at`) assigned
    new_todolist = TodoList.create(list_name: params[:name], list_due_date: params[:due_date])
  end
  ...
  def find_allusers(offset, limit)
    ...
    return User.offset(offset).limit(limit).order(:updated_at)
  end
  ...
  def find_alllists(offset, limit)
    ...
    return TodoList.offset(offset).limit(limit).order(list_due_date: :desc)
  end
  ...
  def find_todolist_byname(name)
    ...
    return TodoList.where(list_name: name).all
  end
